...
#include <WiFi.h>
#include <WebServer.h>
#include "ESP32MotorControl.h"//download the h and cpp file 
...
//const char *ssid = "err";
//const char *password = "43218765";

const char *ssid = "CHT58292_wifi6";
const char *password = "037667265";
...
void cmd1()
{
    String s = HTTP_WEBPAGE;
    s += HTTP_PAGE_END;
    motor.motorForward(motorR, FULLSPEED);
    motor.motorForward(motorL, FULLSPEED);
...
void cmd2()
{
    ...
    s += HTTP_PAGE_END;
    server.send(200, "text/html", s);
    motor.motorReverse(motorR, FULLSPEED);
    motor.motorReverse(motorL, FULLSPEED);
...
void cmd3()
{
    ...
    s += HTTP_PAGE_END;
    server.send(200, "text/html", s);
    motor.motorReverse(motorR, HALFSPEED);
    motor.motorForward(motorL, HALFSPEED);
...
void cmd4()
{
    ...
    s += HTTP_PAGE_END;
    server.send(200, "text/html", s);
    motor.motorForward(motorR, HALFSPEED);
    motor.motorReverse(motorL, HALFSPEED);
...
void setup()
{
    ...
    if (HeadingTgt >= 360)
        HeadingTgt = HeadingTgt - 360;
    else if (HeadingTgt < 0)
        HeadingTgt = HeadingTgt + 360;
...
void loop()
{
    ...
    case CMD_STOP:
        ...
        break;
    case CMD_FORWARD:
        motor.motorForward(motorR,USR_MotorPower - PID_MotorPower);//64-
        motor.motorForward(motorL,USR_MotorPower + PID_MotorPower);//actually right!
        break;
    case CMD_BACKWARD:
